...
function AdminDetailPageManager() {
  ...
  // Handle delete for hotels and cards
  const handleDelete = async (id, type) => {
    if (!window.confirm('Are you sure you want to delete this ' + (type === 'hotel' ? 'hotel' : 'group tour') + '?')) return;
    setError('');
    setSuccess('');
    try {
      const endpoint = type === 'hotel'
        ? `${API_BASE_URL}/api/hotels/${id}`
        : `${API_BASE_URL}/api/cards/${id}`;
      await axios.delete(endpoint);
      setSuccess((type === 'hotel' ? 'Hotel' : 'Group tour') + ' deleted successfully');
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add New Group Tour Button */}
      {activeTab === 'tours' && (
        <div className="flex justify-end mb-4">
          <button
            onClick={() => { setEditingItem(null); setEditType('card'); setShowEditModal(true); }}
            className="bg-blue-600 text-white px-6 py-2 rounded-xl font-semibold shadow hover:bg-blue-700 transition-colors flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {activeTab === 'hotels' ? (
          ...
          cards.map((card) => (
            <div key={card._id} className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden group relative">
              {/* Image */}
              <div className="h-56 relative overflow-hidden">
                ...
                <div className="absolute top-2 right-2 flex gap-2">
                  <button
                    ...
                    className="bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-700 transition-colors shadow"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                  ...
                  <button
                    ...
                    className="bg-red-500 text-white p-2 rounded-lg hover:bg-red-600 transition-colors shadow"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
